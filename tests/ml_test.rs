...
fn test_linear_regression_missing_target_column() {
    let mut columns = BTreeMap::new();
    columns.insert("feature1".to_string(), Series::new_f64("feature1", vec![Some(1.0), Some(2.0)]));
    let df = DataFrame::new(columns).unwrap();
...
fn test_linear_regression_missing_feature_column() {
    let mut columns = BTreeMap::new();
    columns.insert("target".to_string(), Series::new_f64("target", vec![Some(1.0), Some(2.0)]));
    let df = DataFrame::new(columns).unwrap();
...
fn test_linear_regression_with_nulls() {
    let mut columns = BTreeMap::new();
    columns.insert("feature1".to_string(), Series::new_f64("feature1", vec![Some(1.0), None, Some(3.0)]));
    columns.insert("target".to_string(), Series::new_f64("target", vec![Some(2.0), Some(4.0), None]));
    let df = DataFrame::new(columns).unwrap();
...
fn test_linear_regression_single_feature() {
    let mut columns = BTreeMap::new();
    columns.insert("x".to_string(), Series::new_f64("x", vec![Some(1.0), Some(2.0), Some(3.0), Some(4.0)]));
    columns.insert("y".to_string(), Series::new_f64("y", vec![Some(2.0), Some(4.0), Some(6.0), Some(8.0)]));
    let df = DataFrame::new(columns).unwrap();
    ...
    for (pred, actual) in predictions.iter().zip(&[2.0, 4.0, 6.0, 8.0]) {
        assert!((pred - actual).abs() < 0.1, "Prediction {} too far from actual {}", pred, actual);
    }
}
...
fn test_linear_regression_multiple_features() {
    let mut columns = BTreeMap::new();
    columns.insert("x1".to_string(), Series::new_f64("x1", vec![Some(1.0), Some(2.0), Some(3.0), Some(4.0)]));
    columns.insert("x2".to_string(), Series::new_f64("x2", vec![Some(2.0), Some(3.0), Some(4.0), Some(5.0)]));
    columns.insert("y".to_string(), Series::new_f64("y", vec![Some(5.0), Some(8.0), Some(11.0), Some(14.0)])); // y = 2*x1 + 1*x2 + 0
    let df = DataFrame::new(columns).unwrap();
...
fn test_linear_regression_prediction_different_data() {
    ...
    train_columns.insert("x".to_string(), Series::new_f64("x", vec![Some(1.0), Some(2.0), Some(3.0)]));
    train_columns.insert("y".to_string(), Series::new_f64("y", vec![Some(2.0), Some(4.0), Some(6.0)]));
    let train_df = DataFrame::new(train_columns).unwrap();
    
    let mut test_columns = BTreeMap::new();
    test_columns.insert("x".to_string(), Series::new_f64("x", vec![Some(4.0), Some(5.0)]));
