...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  AVX2 Support: {}", veloxx::performance::simd::has_avx2_support());
    println!();
    ...
    println!("SIMD speedup: {:.2}x", traditional_time.as_nanos() as f64 / simd_time.as_nanos() as f64);
    
    // Verify results are equivalent
    println!("Results match: {}", verify_series_equal(&traditional_result, &simd_result));
    println!();
    ...
    println!("Parallel speedup: {:.2}x", traditional_sum_time.as_nanos() as f64 / parallel_sum_time.as_nanos() as f64);
    ...
    let string_series = Series::new_string("cities", vec![
        Some("New York".to_string()), Some("London".to_string()), Some("New York".to_string()),
        Some("Paris".to_string()), Some("London".to_string()), Some("New York".to_string()),
        Some("Tokyo".to_string()), Some("Paris".to_string()), Some("London".to_string()),
    ]);
    
    let bool_series = Series::new_bool("flags", vec![
    ...
    columns.insert("price".to_string(), Series::new_f64("price", 
        (0..10000).map(|i| Some(100.0 + (i as f64 * 0.1))).collect()));
    columns.insert("volume".to_string(), Series::new_i32("volume", 
        (0..10000).map(|i| Some(1000 + i)).collect()));
    columns.insert("symbol".to_string(), Series::new_string("symbol", 
        (0..10000).map(|i| Some(format!("STOCK{}", i % 100))).collect()));
    
    let df = DataFrame::new(columns)?;
    println!("Created DataFrame with {} rows and {} columns", df.row_count(), df.column_count());
    ...
    if let Some(price_series) = df.get_column("price") {
        ...
        println!("Price column parallel mean: {:?} (time: {:?})", par_mean, par_time);
        println!("Price column traditional mean: {:?} (time: {:?})", traditional_mean, traditional_time);
        println!("Parallel mean speedup: {:.2}x", traditional_time.as_nanos() as f64 / par_time.as_nanos() as f64);
    }
...
fn traditional_add(s1: &Series, s2: &Series) -> Result<Series, Box<dyn std::error::Error>> {
    match (s1, s2) {
        (Series::F64(_, a), Series::F64(_, b)) => {
            let result: Vec<Option<f64>> = a.iter().zip(b.iter())
                .map(|(x, y)| match (x, y) {
