...
mod tests {
    ...
    fn test_dataframe_new() {
        ...
        columns.insert("col1".to_string(), Series::new_i32("col1", vec![Some(1), Some(2)]));
        columns.insert("col2".to_string(), Series::new_f64("col2", vec![Some(1.0), Some(2.0)]));
    ...
    fn test_dataframe_display() {
        use std::collections::BTreeMap;
        let mut columns = BTreeMap::new();
        columns.insert("col1".to_string(), Series::new_i32("col1", vec![Some(1), None, Some(3)]));
        columns.insert("col2".to_string(), Series::new_string("col2", vec![Some("a".to_string()), Some("b".to_string()), None]));
        columns.insert("col3".to_string(), Series::new_f64("col3", vec![Some(1.1), Some(2.2), Some(3.3)]));
    ...
    fn test_dataframe_from_vec_of_vec() {
        let data = vec![
            vec!["1".to_string(), "2.0".to_string(), "true".to_string(), "hello".to_string()],
            vec!["4".to_string(), "5.0".to_string(), "false".to_string(), "world".to_string()],
            vec!["7".to_string(), "8.0".to_string(), "".to_string(), "rust".to_string()],
            vec!["".to_string(), "".to_string(), "true".to_string(), "".to_string()],
        ];
        let column_names = vec!["col_i32".to_string(), "col_f64".to_string(), "col_bool".to_string(), "col_string".to_string()];
        ...
        match col_string {
            Series::String(_, v) => assert_eq!(*v, vec![Some("hello".to_string()), Some("world".to_string()), Some("rust".to_string()), None]),
        ...
        assert_eq!(err, "Number of columns in data does not match number of column names.");
    }
    ...
    fn test_dataframe_select_columns() {
        ...
        columns.insert("col1".to_string(), Series::new_i32("col1", vec![Some(1), Some(2)]));
        columns.insert("col2".to_string(), Series::new_f64("col2", vec![Some(1.0), Some(2.0)]));
        columns.insert("col3".to_string(), Series::new_string("col3", vec![Some("a".to_string()), Some("b".to_string())]));

        let df = DataFrame::new(columns).unwrap();

        // Select a subset of columns
        let selected_df = df.select_columns(vec!["col1".to_string(), "col3".to_string()]).unwrap();
        ...
        let err = df.select_columns(vec!["col1".to_string(), "non_existent".to_string()]).unwrap_err();
        assert_eq!(err, "Column 'non_existent' not found.");

        // Select all columns
        let all_columns_df = df.select_columns(vec!["col1".to_string(), "col2".to_string(), "col3".to_string()]).unwrap();
    ...
    fn test_dataframe_drop_columns() {
        ...
        let err = df.drop_columns(vec!["col1".to_string(), "non_existent".to_string()]).unwrap_err();
        assert_eq!(err, "Column 'non_existent' not found.");

        // Drop all columns
        let empty_df = df.drop_columns(vec!["col1".to_string(), "col2".to_string(), "col3".to_string()]).unwrap();
        assert_eq!(empty_df.column_count(), 0);
        assert_eq!(empty_df.row_count(), 0);
    }
    ...
    fn test_dataframe_filter() {
        let mut columns = std::collections::BTreeMap::new();
        columns.insert("age".to_string(), Series::new_i32("age", vec![Some(10), Some(20), Some(30), Some(40)]));
        columns.insert("city".to_string(), Series::new_string("city", vec![Some("London".to_string()), Some("Paris".to_string()), Some("London".to_string()), Some("New York".to_string())]));
        let df = DataFrame::new(columns).unwrap();
        ...
        assert_eq!(filtered_df.get_column("age").unwrap().get_value(0), Some(Value::I32(30)));
        assert_eq!(filtered_df.get_column("age").unwrap().get_value(1), Some(Value::I32(40)));

        // Filter by city == "London"
        let condition = Condition::Eq("city".to_string(), Value::String("London".to_string()));
        let filtered_df = df.filter(&condition).unwrap();
        assert_eq!(filtered_df.row_count(), 2);
        assert_eq!(filtered_df.get_column("city").unwrap().get_value(0), Some(Value::String("London".to_string())));
        assert_eq!(filtered_df.get_column("city").unwrap().get_value(1), Some(Value::String("London".to_string())));
        ...
        let condition = Condition::Or(
            ...
            Box::new(Condition::Eq("city".to_string(), Value::String("Paris".to_string()))),
        );
        let filtered_df = df.filter(&condition).unwrap();
        assert_eq!(filtered_df.row_count(), 2);
        ...
        let filtered_df = df.filter(&condition).unwrap();
        assert_eq!(filtered_df.row_count(), 2);
        assert_eq!(filtered_df.get_column("age").unwrap().get_value(0), Some(Value::I32(10)));
        assert_eq!(filtered_df.get_column("age").unwrap().get_value(1), Some(Value::I32(20)));
    ...
    fn test_dataframe_drop_nulls() {
        ...
        assert_eq!(dropped_df.get_column("col1").unwrap().get_value(0), Some(Value::I32(1)));
        assert_eq!(dropped_df.get_column("col2").unwrap().get_value(0), Some(Value::String("a".to_string())));
        assert_eq!(dropped_df.get_column("col3").unwrap().get_value(0), Some(Value::F64(1.1)));

        let mut columns_no_nulls = std::collections::BTreeMap::new();
        columns_no_nulls.insert("col1".to_string(), Series::new_i32("col1", vec![Some(1), Some(2)]));
    ...
    fn test_dataframe_fill_nulls() {
        ...
        columns.insert("col1".to_string(), Series::new_i32("col1", vec![Some(1), None, Some(3)]));
        columns.insert("col2".to_string(), Series::new_string("col2", vec![Some("a".to_string()), Some("b".to_string()), None]));
        columns.insert("col3".to_string(), Series::new_f64("col3", vec![Some(1.1), Some(2.2), None]));

        let df = DataFrame::new(columns).unwrap();

        // Fill i32 column
        let filled_df_i32 = df.fill_nulls(Value::I32(99)).unwrap();
        assert_eq!(filled_df_i32.get_column("col1").unwrap().get_value(1), Some(Value::I32(99)));

        // Fill string column
        let filled_df_string = df.fill_nulls(Value::String("missing".to_string())).unwrap();
        assert_eq!(filled_df_string.get_column("col2").unwrap().get_value(2), Some(Value::String("missing".to_string())));

        // Fill f64 column
        let filled_df_f64 = df.fill_nulls(Value::F64(99.9)).unwrap();
        assert_eq!(filled_df_f64.get_column("col3").unwrap().get_value(2), Some(Value::F64(99.9)));

        // Test type mismatch for i32 column
        let filled_df_mismatch_i32 = df.fill_nulls(Value::String("wrong_type".to_string())).unwrap();
        assert_eq!(filled_df_mismatch_i32.get_column("col1").unwrap().get_value(1), None); // Should remain None

        // Test type mismatch for string column
        let filled_df_mismatch_string = df.fill_nulls(Value::I32(123)).unwrap();
        assert_eq!(filled_df_mismatch_string.get_column("col2").unwrap().get_value(2), None); // Should remain None

        // Test type mismatch for f64 column
        let filled_df_mismatch_f64 = df.fill_nulls(Value::Bool(true)).unwrap();
        assert_eq!(filled_df_mismatch_f64.get_column("col3").unwrap().get_value(2), None); // Should remain None
    }
    ...
    fn test_series_cast() {
        ...
        // Test string to i32
        let series_string_i32 = Series::new_string("str_int_col", vec![Some("10".to_string()), Some("abc".to_string()), None]);
        ...
        // Test string to f64
        let series_string_f64 = Series::new_string("str_float_col", vec![Some("10.5".to_string()), Some("xyz".to_string()), None]);
        ...
        // Test string to bool
        let series_string_bool = Series::new_string("str_bool_col", vec![Some("true".to_string()), Some("false".to_string()), Some("invalid".to_string()), None]);
        let casted_bool_from_string = series_string_bool.cast(crate::types::DataType::Bool).unwrap();
        ...
        let err = series_i32_unsupported.cast(crate::types::DataType::String).unwrap_err();
        ...
        let casted_i32_same_type = series_i32_same_type.cast(crate::types::DataType::I32).unwrap();
    ...
    fn test_dataframe_sort() {
        let mut columns = std::collections::BTreeMap::new();
        columns.insert("col1".to_string(), Series::new_i32("col1", vec![Some(3), Some(1), Some(2)]));
        columns.insert("col2".to_string(), Series::new_string("col2", vec![Some("c".to_string()), Some("a".to_string()), Some("b".to_string())]));
        let df = DataFrame::new(columns).unwrap();

        // Sort by col1 ascending
        let sorted_df = df.sort(vec!["col1".to_string()], true).unwrap();
        assert_eq!(sorted_df.get_column("col1").unwrap().get_value(0), Some(Value::I32(1)));
        assert_eq!(sorted_df.get_column("col1").unwrap().get_value(1), Some(Value::I32(2)));
        assert_eq!(sorted_df.get_column("col1").unwrap().get_value(2), Some(Value::I32(3)));

        // Sort by col1 descending
        let sorted_df_desc = df.sort(vec!["col1".to_string()], false).unwrap();
        assert_eq!(sorted_df_desc.get_column("col1").unwrap().get_value(0), Some(Value::I32(3)));
        assert_eq!(sorted_df_desc.get_column("col1").unwrap().get_value(1), Some(Value::I32(2)));
        assert_eq!(sorted_df_desc.get_column("col1").unwrap().get_value(2), Some(Value::I32(1)));

        // Sort by col2 ascending
        let sorted_df_str = df.sort(vec!["col2".to_string()], true).unwrap();
        assert_eq!(sorted_df_str.get_column("col2").unwrap().get_value(0), Some(Value::String("a".to_string())));
        assert_eq!(sorted_df_str.get_column("col2").unwrap().get_value(1), Some(Value::String("b".to_string())));
        assert_eq!(sorted_df_str.get_column("col2").unwrap().get_value(2), Some(Value::String("c".to_string())));
