...
impl WasmDataFrame {
    ...
    pub fn new(columns: &js_sys::Object) -> Result<WasmDataFrame, JsValue> {
        ...
        Ok(WasmDataFrame { df: DataFrame::new(rust_columns).map_err(|e| JsValue::from_str(&e))? })
    }
    ...
    pub fn filter(&self, row_indices: Box<[usize]>) -> Result<WasmDataFrame, JsValue> {
        Ok(WasmDataFrame { df: self.df.filter_by_indices(row_indices.as_ref()).map_err(|e| JsValue::from_str(&e))? })
    }
    ...
    pub fn select_columns(&self, names: Box<[JsValue]>) -> Result<WasmDataFrame, JsValue> {
        ...
        Ok(WasmDataFrame { df: self.df.select_columns(names_vec).map_err(|e| JsValue::from_str(&e))? })
    }

    #[wasm_bindgen(js_name = dropColumns)]
    pub fn drop_columns(&self, names: Box<[JsValue]>) -> Result<WasmDataFrame, JsValue> {
        let names_vec: Vec<String> = names.into_iter().map(|s| s.as_string().unwrap_or_default()).collect();
        Ok(WasmDataFrame { df: self.df.drop_columns(names_vec).map_err(|e| JsValue::from_str(&e))? })
    }

    #[wasm_bindgen(js_name = renameColumn)]
    pub fn rename_column(&self, old_name: &str, new_name: &str) -> Result<WasmDataFrame, JsValue> {
        Ok(WasmDataFrame { df: self.df.rename_column(old_name, new_name).map_err(|e| JsValue::from_str(&e))? })
    }

    #[wasm_bindgen(js_name = dropNulls)]
    pub fn drop_nulls(&self) -> Result<WasmDataFrame, JsValue> {
        Ok(WasmDataFrame { df: self.df.drop_nulls().map_err(|e| JsValue::from_str(&e))? })
    }

    #[wasm_bindgen(js_name = fillNulls)]
    pub fn fill_nulls(&self, value: WasmValue) -> Result<WasmDataFrame, JsValue> {
        Ok(WasmDataFrame { df: self.df.fill_nulls(value.value).map_err(|e| JsValue::from_str(&e))? })
    }
    ...
    pub fn sort(&self, by_columns: Box<[JsValue]>, ascending: bool) -> Result<WasmDataFrame, JsValue> {
        ...
        Ok(WasmDataFrame { df: self.df.sort(by_columns_vec, ascending).map_err(|e| JsValue::from_str(&e))? })
    }
}
...
impl WasmSeries {
    ...
    pub fn filter(&self, row_indices: Box<[usize]>) -> Result<WasmSeries, JsValue> {
        Ok(WasmSeries { series: self.series.filter(row_indices.as_ref()).map_err(|e| JsValue::from_str(&e))? })
    }

    #[wasm_bindgen(js_name = fillNulls)]
    pub fn fill_nulls(&self, value: WasmValue) -> Result<WasmSeries, JsValue> {
        Ok(WasmSeries { series: self.series.fill_nulls(&value.value).map_err(|e| JsValue::from_str(&e))? })
    }

    #[wasm_bindgen]
    pub fn cast(&self, to_type: WasmDataType) -> Result<WasmSeries, JsValue> {
        Ok(WasmSeries { series: self.series.cast(to_type.into()).map_err(|e| JsValue::from_str(&e))? })
    }

    #[wasm_bindgen]
    pub fn append(&self, other: &WasmSeries) -> Result<WasmSeries, JsValue> {
        Ok(WasmSeries { series: self.series.append(&other.series).map_err(|e| JsValue::from_str(&e))? })
    }

    #[wasm_bindgen]
    pub fn sum(&self) -> Result<JsValue, JsValue> {
        Ok(self.series.sum().map_err(|e| JsValue::from_str(&e))?.map_or(JsValue::NULL, |v| match v {
    ...
    pub fn min(&self) -> Result<JsValue, JsValue> {
        Ok(self.series.min().map_err(|e| JsValue::from_str(&e))?.map_or(JsValue::NULL, |v| match v {
            Value::I32(val) => JsValue::from_f64(val as f64),
            Value::F64(val) => JsValue::from_f64(val),
            Value::DateTime(val) => JsValue::from_f64(val as f64),
            _ => JsValue::NULL,
    ...
    pub fn max(&self) -> Result<JsValue, JsValue> {
        Ok(self.series.max().map_err(|e| JsValue::from_str(&e))?.map_or(JsValue::NULL, |v| match v {
            Value::I32(val) => JsValue::from_f64(val as f64),
            Value::F64(val) => JsValue::from_f64(val),
            Value::DateTime(val) => JsValue::from_f64(val as f64),
            _ => JsValue::NULL,
    ...
    pub fn mean(&self) -> Result<JsValue, JsValue> {
        Ok(self.series.mean().map_err(|e| JsValue::from_str(&e))?.map_or(JsValue::NULL, |v| match v {
            Value::F64(val) => JsValue::from_f64(val),
            _ => JsValue::NULL,
        }))
    }
    ...
    pub fn median(&self) -> Result<JsValue, JsValue> {
        Ok(self.series.median().map_err(|e| JsValue::from_str(&e))?.map_or(JsValue::NULL, |v| match v {
            Value::I32(val) => JsValue::from_f64(val as f64),
            Value::F64(val) => JsValue::from_f64(val),
            Value::DateTime(val) => JsValue::from_f64(val as f64),
            _ => JsValue::NULL,
    ...
    pub fn std_dev(&self) -> Result<JsValue, JsValue> {
        Ok(self.series.std_dev().map_err(|e| JsValue::from_str(&e))?.map_or(JsValue::NULL, |v| match v {
            Value::F64(val) => JsValue::from_f64(val),
            _ => JsValue::NULL,
        }))
    }
    ...
    pub fn correlation(&self, other: &WasmSeries) -> Result<JsValue, JsValue> {
        Ok(self.series.correlation(&other.series).map_err(|e| JsValue::from_str(&e))?.map_or(JsValue::NULL, |v| match v {
            Value::F64(val) => JsValue::from_f64(val),
            _ => JsValue::NULL,
        }))
    }
    ...
    pub fn covariance(&self, other: &WasmSeries) -> Result<JsValue, JsValue> {
        Ok(self.series.covariance(&other.series).map_err(|e| JsValue::from_str(&e))?.map_or(JsValue::NULL, |v| match v {
            Value::F64(val) => JsValue::from_f64(val),
            _ => JsValue::NULL,
        }))
    }
    ...
    pub fn unique(&self) -> Result<WasmSeries, JsValue> {
        Ok(WasmSeries { series: self.series.unique().map_err(|e| JsValue::from_str(&e))? })
    }

    #[wasm_bindgen(js_name = toVecF64)]
    pub fn to_vec_f64(&self) -> Result<Box<[JsValue]>, JsValue> {
        let vec_f64 = self.series.to_vec_f64().map_err(|e| JsValue::from_str(&e))?;
    ...
    pub fn interpolate_nulls(&self) -> Result<WasmSeries, JsValue> {
        Ok(WasmSeries { series: self.series.interpolate_nulls().map_err(|e| JsValue::from_str(&e))? })
    }
}
