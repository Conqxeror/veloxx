...
fn bench_dataframe_creation(c: &mut Criterion) {
    c.bench_function("dataframe_creation", |b| {
        b.iter(|| {
            ...
            columns.insert("col1".to_string(), Series::new_i32("col1", vec![Some(1); 1000]));
            columns.insert("col2".to_string(), Series::new_f64("col2", vec![Some(1.0); 1000]));
...
fn bench_dataframe_filter(c: &mut Criterion) {
    let mut columns = BTreeMap::new();
    columns.insert("col1".to_string(), Series::new_i32("col1", (0..1000).map(Some).collect()));
    columns.insert("col2".to_string(), Series::new_f64("col2", (0..1000).map(|i| Some(i as f64)).collect()));
...
fn bench_dataframe_sort(c: &mut Criterion) {
    let mut columns = BTreeMap::new();
    columns.insert("col1".to_string(), Series::new_i32("col1", (0..1000).rev().map(Some).collect()));
    columns.insert("col2".to_string(), Series::new_f64("col2", (0..1000).map(|i| Some(i as f64)).collect()));
    let df = DataFrame::new(columns).unwrap();
...
fn bench_dataframe_join(c: &mut Criterion) {
    ...
    columns1.insert("id".to_string(), Series::new_i32("id", (0..1000).map(Some).collect()));
    columns1.insert("value1".to_string(), Series::new_i32("value1", (0..1000).map(Some).collect()));
    let df1 = DataFrame::new(columns1).unwrap();

    let mut columns2 = BTreeMap::new();
    columns2.insert("id".to_string(), Series::new_i32("id", (0..1000).map(Some).collect()));
    columns2.insert("value2".to_string(), Series::new_i32("value2", (0..1000).map(Some).collect()));
...
fn bench_series_apply(c: &mut Criterion) {
    ...
    c.bench_function("series_apply_i32", |b| {
        b.iter(|| {
            series_i32.apply(|v| v.map(|val| match val {
    ...
    c.bench_function("series_apply_f64", |b| {
        b.iter(|| {
            series_f64.apply(|v| v.map(|val| match val {
    ...
    c.bench_function("series_apply_string", |b| {
        b.iter(|| {
            series_string.apply(|v| v.map(|val| match val {
                velox::types::Value::String(s) => velox::types::Value::String(format!("{s}-suffix")),
                _ => panic!("Unexpected type"),
            })).unwrap();
        });
    });
...
fn bench_dataframe_from_vec_of_vec(c: &mut Criterion) {
    let data: Vec<Vec<String>> = (0..1000).map(|i| vec![i.to_string(), format!("name{}", i), (i % 2 == 0).to_string()]).collect();
...
fn bench_dataframe_drop_nulls(c: &mut Criterion) {
    ...
    let data_with_nulls_f64: Vec<Option<f64>> = (0..1000)
        ...
        .collect();
    columns.insert("col1".to_string(), Series::new_i32("col1", data_with_nulls_i32));
    columns.insert("col2".to_string(), Series::new_f64("col2", data_with_nulls_f64));
    let df = DataFrame::new(columns).unwrap();
...
fn bench_series_cast(c: &mut Criterion) {
    let series_i32 = Series::new_i32("data", (0..1000).map(Some).collect());
    let series_f64 = Series::new_f64("data", (0..1000).map(|i| Some(i as f64)).collect());
    let series_string_i32 = Series::new_string("data", (0..1000).map(|i| Some(i.to_string())).collect());
    let series_string_f64 = Series::new_string(
        "data",
        (0..1000).map(|i| Some(format!("{i}.0"))).collect(),
    );
...
fn bench_dataframe_with_column(c: &mut Criterion) {
    let mut columns = BTreeMap::new();
    columns.insert("col1".to_string(), Series::new_i32("col1", (0..1000).map(Some).collect()));
    columns.insert("col2".to_string(), Series::new_i32("col2", (0..1000).map(Some).collect()));
    let df = DataFrame::new(columns).unwrap();
...
fn bench_dataframe_append(c: &mut Criterion) {
    let mut columns1 = BTreeMap::new();
    columns1.insert("col1".to_string(), Series::new_i32("col1", (0..500).map(Some).collect()));
    columns1.insert("col2".to_string(), Series::new_f64("col2", (0..500).map(|i| Some(i as f64)).collect()));
    let df1 = DataFrame::new(columns1).unwrap();

    let mut columns2 = BTreeMap::new();
    columns2.insert("col1".to_string(), Series::new_i32("col1", (500..1000).map(Some).collect()));
    columns2.insert("col2".to_string(), Series::new_f64("col2", (500..1000).map(|i| Some(i as f64)).collect()));
    let df2 = DataFrame::new(columns2).unwrap();
...
fn bench_dataframe_correlation(c: &mut Criterion) {
    let mut columns = BTreeMap::new();
    columns.insert("col1".to_string(), Series::new_i32("col1", (0..1000).map(Some).collect()));
    columns.insert("col2".to_string(), Series::new_i32("col2", (0..1000).map(|i| Some(i * 2)).collect()));
    let df = DataFrame::new(columns).unwrap();
...
fn bench_series_interpolate_nulls(c: &mut Criterion) {
    let series_i32 = Series::new_i32("data", (0..1000).map(|i| if i % 5 == 0 { None } else { Some(i) }).collect());
...
criterion_group!(benches, bench_dataframe_creation, bench_dataframe_from_csv, bench_dataframe_filter, bench_series_sum, bench_dataframe_sort, bench_dataframe_join, bench_series_apply, bench_dataframe_from_vec_of_vec, bench_dataframe_drop_nulls, bench_dataframe_fill_nulls, bench_series_cast, bench_dataframe_with_column, bench_dataframe_append, bench_dataframe_describe, bench_dataframe_correlation, bench_dataframe_covariance, bench_series_unique, bench_series_interpolate_nulls, bench_dataframe_from_json);
